...
void test_mapreduce_tasks() {
    ...
    std::cout << std::right << std::setw(8) << std::setfill(' ') << "size" << std::right << std::setw(8) << "s map" << std::right
              << std::setw(8) << "p1 map" << std::right << std::setw(8) << "p2 map" << std::right << std::setw(8) << "s fold" << std::right
              << std::setw(8) << "p1 fold" << std::right << std::setw(8) << "p2 fold" << std::endl;
    ...
    for (auto const size : sizes) {
        ...
        auto s2 = 0LL;
        auto tp1m =
            perf_timer<>::duration([&] { version1::parallel_map(std::begin(v2), std::end(v2), [](int const i) { return i + i; }); });
        auto tp1f = perf_timer<>::duration([&] { s2 = version1::parallel_reduce(std::begin(v2), std::end(v2), 0LL, std::plus<>()); });

        auto v3 = v;
        auto s3 = 0LL;
        auto tp2m =
            perf_timer<>::duration([&] { version2::parallel_map(std::begin(v3), std::end(v3), [](int const i) { return i + i; }); });
        auto tp2f = perf_timer<>::duration([&] { s3 = version2::parallel_reduce(std::begin(v3), std::end(v3), 0LL, std::plus<>()); });

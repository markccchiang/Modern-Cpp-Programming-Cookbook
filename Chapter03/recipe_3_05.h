...
void execute() {
    {
        ...
        auto m = funclib::mapf(
            [](std::pair<std::string, int> const kvp) { return std::make_pair(funclib::mapf(toupper, kvp.first), kvp.second); }, words);
...

    {
        ...
        auto words = std::map<std::string, int>{{"one", 1}, {"two", 2}, {"three", 3}};
        auto count = funclib::foldl([](int const s, std::pair<std::string, int> const kvp) { return s + kvp.second; }, words, 0);
...

    {
        ...
        auto s = funclib::foldl(std::plus<>(),
            funclib::mapf([](int const i) { return i + i; }, funclib::mapf([](int const i) { return std::abs(i); }, vnums)), 0);
        // s = 76
    }
}
} // namespace recipe_3_05
